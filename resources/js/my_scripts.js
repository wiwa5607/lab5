...
function loadStatsPage() {
	...
	var myTab = document.getElementById('stats_table');
	var wins = document.getElementById('wins');
	var losses = document.getElementById('losses');
	for (i = 2; i < myTab.rows.length; i++) {
		var row = myTab.rows.item(i).cells;
		var buffs = parseInt(row.item(2).innerHTML);
		var away = parseInt(row.item(3).innerHTML);
		if (buffs > away) {
			row.item(4).innerHTML = "Colorado";
			wins.innerHTML++;
		}
		else {
			row.item(4).innerHTML = row.item(1).innerHTML;
			losses.innerHTML++;
		}
	}
